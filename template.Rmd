...
```{r colnames_to_labels, include=FALSE}
#utility funciton to change generic county and state column names to the labels
colnames_to_labels = function(data) {
  data %>% 
    rename((!!params$county) := County,
           (!!params$state) := State)
}
```
...
hh_size_vars <- c(
  "Average household size_2020",
  "Average household size_2010",
  "Average household size_2000",
  "Average household size_1990"
)

#population comparison boolean
if(nrow(params$muni_compare) > 0) {
  population_compare <- TRUE 
} else {
  population_compare <- FALSE 
}
...
```{r cover_image,echo = FALSE}
#doesn't work locally, uncomment when deploying
external_img(src = "RPC_Web_RGB_Horizontal.jpg", height = 1.3125/2, width = 2.90625/2)
```

<br>
<br>

**`r params$muni`: Census Data Report* **  
Generated on `r Sys.Date()` using the [CARPC Census Cruncher](https://capital-area-rpc.shinyapps.io/planning_report_tool_pilot/)

<br>
<br>

*Note that for a municipality split between multiple counties, this report only includes census data for the area within `r params$county`
...
<br>
<br>
Note that historic population data is not included for municipalities that have changed type. For example, if a town became a city in 2010, then the population totals will show as NA prior to 2010.
...
```{r population_comparison_table, echo=FALSE, eval=population_compare}

pop_compare <- pop_data %>% 
  slice(1) %>% 
  select(-`Population Change 2010-2020`) %>% 
  rename(Municipality = name) %>% 
  bind_rows(params$muni_compare)

#display the table
flextable(pop_compare) %>%
  set_caption("Population by Decade - Municipal Comparison") %>% 
  autofit(add_w = 0)
...
`r if(population_compare){"Source: 2020, 2010, 2000, 1990, 1980 Decennial census data"}`

`r if(population_compare){"\\newpage"}`
...
age_data <- full_join(age_data_s1, age_data_s2, join_by(Variable == Variable)) %>% 
  #order correctly
  select(Variable, !!as.name(paste0("2010_", params$muni)), !!as.name(paste0("2020_", params$muni)), `2010_County`, `2020_County`, `2010_State`, `2020_State`)
...
age_table <- flextable(age_data) %>%
  ...
  add_header_row(
  colwidths = c(1, 2, 2, 2),
  values = c("", params$muni, params$county, params$state)
  ) %>%
...
age_graph_data_percent_table_s1 <- params$data %>%
  ...
  summarise(`2020_community` = sum(!!as.name(params$muni)), `2020_County` = sum(County), `2020_State` = sum(State)) 
...
age_graph_data_percent_table_s2 <- params$data %>%
  ...
  group_by(Variable) %>% 
  #sum into brackets, dropping gender division
  summarise(`2010_community` = sum(!!as.name(params$muni)), `2010_County` = sum(County), `2010_State` = sum(State)) 
...
age_graph_data_percent_table <- left_join(age_graph_data_percent_table_s1, age_graph_data_percent_table_s2, join_by(Variable == Variable)) %>% 
  #order correctly
  select(Variable, `2010_community`, `2020_community`, `2010_County`, `2020_County`, `2010_State`, `2020_State` )
...
age_p_county <-
  ms_barchart(
  data = age_graph_data_percent %>%
    select(Variable, County, group) %>% 
    #order correctly
    mutate(Variable = factor(Variable, levels=unique(as.character(Variable)))),
  x = "Variable",
  y = "County", group = "group"
  ) %>% 
  chart_settings(dir = "horizontal", overlap = -50) %>% 
  chart_labels(title = paste0("Female & Male Percent Population by Age Group (", params$county, ")"), xlab = "Age Group", ylab = "Percent of Population") %>% 
  set_theme(chart_default_theme) %>% 
  chart_theme(title_x_rot = 270, title_y_rot = 0) %>% 
  chart_ax_y(num_fmt = "#,###.0\\%")
...
age_graph_data_percent_dual <- age_graph_data_percent %>%
  select(Variable, County, group) %>%
  #make male category values negative to format graph correctly
  mutate(County = case_when(group == "Male" ~ County * -1,
                                   TRUE ~ County))
...
age_p_state <-
  ...
  chart_settings(dir = "horizontal", overlap = -50) %>% 
  chart_labels(title = paste0("Female & Male Percent Population by Age Group (", params$state, ")"), xlab = "Age Group", ylab = "Percent of Population") %>% 
  set_theme(chart_default_theme) %>% 
  chart_theme(title_x_rot = 270, title_y_rot = 0) %>% 
  chart_ax_y(num_fmt = "#,###.0\\%")
...
age_graph_data_percent_dual <- age_graph_data_percent %>%
  select(Variable, State, group) %>% 
  #make male category values negative to format graph correctly
  mutate(State = case_when(group == "Male" ~ State * -1,
                                   TRUE ~ State))
...
race_chart_nh <-
  ...
  data = race_data_nh %>% 
    pivot_longer(2:4) %>% 
    mutate(name = factor(name, levels=c(params$state, params$county, params$muni))), 
  x = "name",
...
median_income_chart <-
  ...
  data = median_income_data %>% 
    mutate(name = factor(name, levels=c(params$muni, params$county, params$state))),
  x = "Variable",
  y = "value", group = "name"
  ) %>%
...
```{r hh_size_table, echo=FALSE}
hh_size_data <- params$data %>%
  filter(Variable %in% hh_size_vars) %>% 
  select(1:2) %>% 
  mutate(Variable = case_when(Variable == "Average household size_2020"~ "2020 average household size",
                              Variable == "Average household size_2010" ~ "2010 average household size",
                              Variable == "Average household size_2000" ~ "2000 average household size",
                              Variable == "Average household size_1990" ~ "1990 average household size"))

#display the table
#head(cost_burden_data, n=length(cost_burden_vars))
flextable(hh_size_data) %>% 
  set_caption("Average Household Size 1990-2020") %>% 
  autofit(add_w = 0)

iterate_progress_bar()
```
Source: 2020 5-year ACS data & 2010, 2000, 1990 Decennial Census data

\newpage

```{r hh_size_graph, echo=FALSE}
...
hh_size_chart <-
  ...
  chart_labels(title = "Average Household Size 1990-2020", xlab = "", ylab = "Average household size") %>%
  set_theme(chart_default_theme) %>% 
  chart_theme(legend_position = "n") %>% 
  #chart_ax_y(num_fmt = "#,##0\\%") %>% 
  #chart_data_labels(num_fmt = "0.0\\%", position = "outEnd", show_val = T) %>% 
...
insert_chart(hh_size_chart, "hh_size")
