...
#from https://www.esri.com/about/newsroom/arcuser/the-importance-of-margins-of-error-and-mapping/:
#If the MOE is less than 12 percent of the estimate, its reliability is considered high (REL = 1). Between 12 and 40 percent, reliability is considered medium (REL = 2). If MOE is more than 40 percent, reliability is considered low (REL = 3).

#set parameters for pulling census data ----
...
#the variables to pull from the decennial census ----
...
census_data <- get_acs(
  ...
  variables = decennial_var_avg_hh,
...
#the variables to pull from the ACS census ----

v20 <- load_variables(2020, "dpas", cache = TRUE)
...
# utility functions for acs data ----
...
#v20 <- load_variables(2010, "sf1", cache = TRUE)
...
# process the historic population data----
#downloaded from IPUMS NHGIS
...
# join all the census estimate datasets----
...
# join the different census datasets together to create MOE data ----
...
# municipality choices and crosswalk ----
...
#code from before utility method
# census_data_step1 <- get_acs(
#   geography = geo_type,
#   variables = acs_var_education,
#   year = acs_year,
...
#   #calculate aggregate estimates
#   mutate(
#     est_less_9th = B15003_002E+B15003_003E+B15003_004E+B15003_005E+B15003_006E+B15003_007E+B15003_008E+B15003_009E+B15003_010E+B15003_011E+B15003_012E,
#     est_no_diploma_9th_12th = B15003_013E+B15003_014E+B15003_015E+B15003_016E,
#     est_high_school_equivalent = B15003_017E+B15003_018E,
#     est_some_college = B15003_019E+B15003_020E,
#     est_associate = B15003_021E,
#     est_bachelor = B15003_022E,
#     est_graduate = B15003_023E+B15003_024E+B15003_025E) %>%
#   #calculate aggregate moes
#   rowwise() %>%
#   mutate(
#     moe_less_9th = moe_sum(moe = c(B15003_002M, B15003_003M, B15003_004M, B15003_005M, B15003_006M, B15003_007M, B15003_008M, B15003_009M, B15003_010M, B15003_011M, B15003_012M), estimate = c(B15003_002E, B15003_003E, B15003_004E, B15003_005E, B15003_006E, B15003_007E, B15003_008E, B15003_009E, B15003_010E, B15003_011E, B15003_012E)),
#     moe_no_diploma_9th_12th = moe_sum(moe = c(B15003_013M, B15003_014M, B15003_015M, B15003_016M), estimate = c(B15003_013E, B15003_014E, B15003_015E, B15003_016E)),
#     moe_high_school_equivalent = moe_sum(moe = c(B15003_017M, B15003_018M), estimate = c(B15003_017E, B15003_018E)),
#     moe_some_college = moe_sum(moe = c(B15003_019M, B15003_020M), estimate = c(B15003_019E, B15003_020E)),
#     moe_associate = B15003_021M,
#     moe_bachelor = B15003_022M,
#     moe_graduate = moe_sum(moe = c(B15003_023M, B15003_024M, B15003_025M), estimate = c(B15003_023E, B15003_024E, B15003_025E))
#   ) %>%
#   select(GEOID, NAME, B15003_001E, B15003_001M, 53:66)


# acs_year <- 2022
# v20 <- load_variables(2022, "acs5/profile", cache = TRUE)
# 
# #the year of the current decennial census
# decennial_cur <- 2020
# decennial_past <- decennial_cur - 10
# 
# #the 5 decennial census years before the current one
# #TODO tidy census only goes back to 2000 - need to figure out how to get the historical population data - maybe ipums?
# #look at the data(pums_variables) command in tidycensus
# decennial_historic <- c(decennial_cur-10, decennial_cur-20, decennial_cur-30, decennial_cur-40, decennial_cur-50)
# 
# #geography parameters
# state <- c("WI")
# county <- c("Dane")
# 
# #the variables to pull for historic data
# decennial_var_historic <- c(
#   #population
#   "P1_001N"
# )
# 
# #the variables to pull for last decade
# decennial_var_last <- c(
#   #population
#   "P1_001N"
# )
# 
...
# 
# 
# #the variables to pull from the most recent decennial census
# decennial_var_race <- c(
#   "P2_005N", # white nh
#   "P2_006N", # black nh
#   "P2_007N", # amin nh
...
# )
# 
# #TODO try downloading in wide format, output="wide"
...
# 
# decennial_data_age_cur <- get_decennial_gender_age("county subdivision", decennial_cur)
# #TODO different variables for 2010
# #decennial_data_age_past <- get_decennial_gender_age("county subdivision", decennial_past)
# decennial_data_age_county <- get_decennial_gender_age("county", decennial_cur)
# decennial_data_age_state <- get_decennial_gender_age("state", decennial_cur)
# 
...
# decennial_data_race_state <- get_decennial_race("state", decennial_cur)
# 
# #TODO ignoring margin of error for now, should deal with it eventually
# #educational info
# get_acs_education <- function(geo_type, year) {
# 
#   census_data <- get_acs(
...
#     geometry = F
#   ) %>%
#     #for now just ignore margin of error
#      select(GEOID, NAME, ends_with("E")) %>%
#      #calculate totals
#      mutate(
#        less_9th = B15003_002E+B15003_003E+B15003_004E+B15003_005E+B15003_006E+B15003_007E+B15003_008E+B15003_009E+B15003_010E+B15003_011E+B15003_012E,
#        no_diploma_9th_12th = B15003_013E+B15003_014E+B15003_015E+B15003_016E,
#        high_school_equivalent = B15003_017E+B15003_018E,
#        some_college = B15003_019E+B15003_020E,
#        associate = B15003_021E,
#        bachelor = B15003_022E,
#        graduate = B15003_023E+B15003_024E+B15003_025E) %>%
#      #make percentages
#      mutate(across(28:34, ~ .x / B15003_001E * 100)) %>%
#      #drop original estimates
#      select(GEOID, NAME, less_9th, no_diploma_9th_12th, high_school_equivalent, some_college, associate, bachelor, graduate)
# 
#   return(census_data)
# 
# }
...
#   ) %>% 
#     left_join(var_names %>% select(name, label), join_by(variable == name)) %>% 
#     #separate_wider_delim(label, "!!", names=c("a", "b", "group", "var"), too_few = "debug")
#     mutate(label = case_when(variable %in% c("B19013_001", "B19113_001") ~ substr(label, 11, 100),
#                               TRUE ~ substr(label, 19, 100))) %>%
#     mutate(estimate = case_when(variable %in% c("B19013_001", "B19113_001") ~ estimate,
#                              TRUE ~ estimate/summary_est * 100)) %>% 
...
# acs_data_income_state <- get_acs_income("state", acs_year)
# 
# #TODO - come back to this one, I don't understand what should be the denominator of these %s
# #poverty info
# get_acs_poverty <- function(geo_type, year) {
#   
#   census_data <- get_acs(
...
#     geometry = F
#   ) #%>% 
#     mutate(estimate = estimate/summary_est * 100) %>% 
#     select(GEOID, NAME, variable, estimate) %>% 
#     pivot_wider(names_from = variable, values_from = estimate) %>% 
#     rename(`poverty Under 18 years` = S1701_C02_002,
#            `poverty 18 to 64 years` = S1701_C02_006,
#            `poverty 65 years and over` = S1701_C02_010)
#   
#   return(census_data)
# }
# 
...
#     geometry = F
#   ) %>% 
#   mutate(estimate = case_when(variable != "S2301_C04_001" ~ estimate/summary_est * 100,
#                               TRUE ~ estimate)) %>% 
#     left_join(var_names %>% select(name, label), join_by(variable == name)) %>%
#     mutate(label = case_when(variable != "S2301_C04_001" ~ str_replace(substr(label, 66, 200), ":", ""),
#                              TRUE ~ "Percent unemployment")) %>% 
...
#     geometry = F
#   ) %>%
#     mutate(`Renters` = (DP04_0141E + DP04_0142E) / DP04_0136E * 100) %>% 
#     mutate(`Mortgage holders` = (DP04_0114E + DP04_0115E) / DP04_0093E * 100) %>% 
#     mutate(`Owned home` = (DP04_0123E + DP04_0124E) / DP04_0102E * 100) %>% 
#     select(GEOID, NAME, `Renters`, `Mortgage holders`, `Owned home`)
#   
#   return(census_data)
# }
# 
...
# acs_data_cost_burden_state <- get_acs_cost_burden("state", acs_year)
# 
# #join the different census datasets together
# data_muni <- decennial_data_age_cur %>%
#   left_join(decennial_data_age_graph_cur, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(decennial_data_race_cur, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_education_cur, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_income_cur, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_employment_cur, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_structure_year_cur, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_homeowner_cur, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_home_value_cur, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_cost_burden_cur, join_by(GEOID == GEOID, NAME == NAME))
# 
# data_county <- decennial_data_age_county %>%
#   left_join(decennial_data_age_graph_county, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(decennial_data_race_county, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_education_county, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_income_county, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_employment_county, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_structure_year_county, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_homeowner_county, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_home_value_county, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_cost_burden_county, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   select(-GEOID) %>%
#   mutate(NAME = "Dane County")
# 
# data_state <- decennial_data_age_state %>%
#   left_join(decennial_data_age_graph_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(decennial_data_race_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_education_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_income_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_employment_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_structure_year_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_homeowner_state, join_by(GEOID == GEOID, NAME == NAME)) %>%
#   left_join(acs_data_home_value_state, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   left_join(acs_data_cost_burden_state, join_by(GEOID == GEOID, NAME == NAME)) %>% 
#   select(-GEOID) %>%
#   mutate(NAME = "State of Wisconsin")
# 
# #the municipal borders
