...
#TODO - can I delete this? Seemed to be multiple data sources...
#average hh size info
...
#average hh size info
get_acs_avg_hh_size <- function(geo_type, year) {
  
  census_data <- get_acs(
    geography = geo_type,
    variables = "B25010_001",
    #we want the data from 2020, not the most current
    year = 2020,
    state = state,
    county = county,
    survey = "acs5",
    ...
    mutate(variable = paste0("Average household size_2020")) %>% 
    rename(estimate_prop = estimate,
           moe_prop = moe) %>% 
    get_cv_reliability()
...
#only need muni data
acs_data_avg_hh_size_cur <- get_acs_avg_hh_size("county subdivision", acs_year)
```
...
get_decennial_avg_hh_size_old <- function(geo_type, year) {
  ...
  census_data <- get_decennial(
    ...
    select(-variable) %>% 
    rename(`Avg household size` = value)
  
  return(census_data)
}

#only need muni data
decennial_data_avg_hh_size_cur_2010 <- get_decennial_avg_hh_size_old("county subdivision", decennial_last) %>% 
  rename(`Average household size_2010` = `Avg household size`)

decennial_data_avg_hh_size_cur_2000 <- get_decennial_avg_hh_size_old("county subdivision", 2000) %>% 
  rename(`Average household size_2000` = `Avg household size`)



```

Process the historic data downloaded from IPUMS NHGIS
...
pop_1970 <- read.csv(paste0(here::here(), "/data/historic_population/nhgis0003_ds94_1970_cty_sub.csv")) %>% 
  filter(STATE == state_full & COUNTY == county) %>% 
  ...
  #will likely need to edit below lines outside of Dane County
  mutate(CTY_SUB = case_when(CTY_SUB == "De Forest village" ~ "DeForest village",
                             CTY_SUB == "Mc Farland village" ~ "McFarland village",
                             TRUE ~ CTY_SUB))
...
historic_pop_muni <- pop_1970 %>% 
  ...
  mutate(CTY_SUB = paste0(CTY_SUB, ", ", county_label, ", ", state_full))
...
historic_pop_state <- read.csv(paste0(here(), "/data/historic_population/state_pop.csv"))

#process the historic hh size data
#note that for state outside of wisconsin, you'll need to change this
hh_size_1990 <- read.csv(paste0(here(), "/data/historic_hh_size/nhgis0006_csv/nhgis0006_ds120_1990_cty_sub.csv")) %>% filter(COUNTY == county) %>% 
  select(CTY_SUB, ESJ001) %>% 
  mutate(CTY_SUB = paste0(CTY_SUB, ", ", county_label, ", Wisconsin")) %>% 
  rename(`Average household size_1990` = ESJ001)

historic_hh_size_muni <- acs_data_avg_hh_size_cur$estimate %>% 
  left_join(decennial_data_avg_hh_size_cur_2010, by = join_by(GEOID == GEOID, NAME == NAME)) %>% 
  left_join(decennial_data_avg_hh_size_cur_2000, by = join_by(GEOID == GEOID, NAME == NAME)) %>% 
  left_join(hh_size_1990, by = join_by(NAME == CTY_SUB)) %>% 
  select(-GEOID)
...
data_muni <- decennial_data_age_cur %>%
  ...
  left_join(historic_hh_size_muni, join_by(NAME == NAME)) %>% 
...
moe_report_muni <- bind_rows(
  list(
    acs_data_education_cur$moe,
    acs_data_income_cur$moe,
    acs_data_employment_cur$moe,
    acs_data_structure_year_cur$moe,
    acs_data_homeowner_cur$moe,
    acs_data_home_value_cur$moe,
    acs_data_cost_burden_cur$moe,
    acs_data_labor_cur$moe,
    acs_data_poverty_cur$moe,
    acs_data_structure_type_cur$moe,
    acs_data_vacancy_cur$moe,
    acs_data_avg_hh_size_cur$moe
  )
) %>%
  clean_moe_table()

moe_report_county <- bind_rows(
  list(
    acs_data_education_county$moe,
    acs_data_income_county$moe,
    acs_data_employment_county$moe,
    acs_data_structure_year_county$moe,
    acs_data_homeowner_county$moe,
    acs_data_home_value_county$moe,
    acs_data_cost_burden_county$moe,
    acs_data_labor_county$moe,
    acs_data_poverty_county$moe,
    acs_data_structure_type_county$moe,
    acs_data_vacancy_county$moe
  )
) %>%
  select(-GEOID) %>%
  mutate(NAME = county_label) %>%
...
moe_report_state <- bind_rows(
  list(
    acs_data_education_state$moe,
    acs_data_income_state$moe,
    acs_data_employment_state$moe,
    acs_data_structure_year_state$moe,
    acs_data_homeowner_state$moe,
    acs_data_home_value_state$moe,
    acs_data_cost_burden_state$moe,
    acs_data_labor_state$moe,
    acs_data_poverty_state$moe,
    acs_data_structure_type_state$moe,
    acs_data_vacancy_state$moe
  )
) %>%
  select(-GEOID) %>%
  mutate(NAME = state_label) %>%
...
Create crosswalk to translate normal muni names to census names

```{r crosswalk}
#translate normal muni names to census names
muni_crosswalk <- data_muni %>%
  select(NAME) %>%
  distinct() %>%
